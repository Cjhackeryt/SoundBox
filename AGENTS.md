...
This repository is a Macro Deck 3 out-of-process plugin named SoundBox. It is not the
Macro Deck plugin template repository. The plugin provides Windows audio playback actions and is
published from this repository.

Worked examples of additional capabilities live in the
...
[README.md](README.md) is the human-facing guide: how to build, run and pack this plugin. This file is
the rule set for writing the plugin. Read it before changing code. The template package itself is covered by
[packaging/README.md](https://github.com/Macro-Deck-App/Macro-Deck-Plugin-Template/blob/main/packaging/README.md).

## Orientation

```
Program.cs               builder chain and RunAsync
manifest.json            identity, repository, icon and entrypoint
macrodeck-build.json     publish target for win-x64
PluginIntegration.cs     integration lifecycle and action registration
Actions/                  playback and stop actions
Localization/Strings.resx default-culture strings
Assets/icon.svg           the manifest icon
```

The upstream template repository carries `.template.config/` and `packaging/`; this generated plugin
does not. Changes here apply directly to the SoundBox project and its single `manifest.json` and
